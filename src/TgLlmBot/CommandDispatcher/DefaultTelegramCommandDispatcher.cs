...
public partial class DefaultTelegramCommandDispatcher : ITelegramCommandDispatcher
{
    ...
    public async Task HandleMessageAsync(Message? message, UpdateType type, CancellationToken cancellationToken)
    {
        ...
        // Регистрируем альбом даже без вложений: по этой отметке обработчик ответа понимает,
        ...
        if (!isBotOwnMessage && hasSupportedMedia)
        {
            // Любое сообщение с вложениями ложится в историю: компактное описание готовит
            // фоновый воркер - оно нужно на будущее, когда вложение уйдёт из актуального
            // контекста. Ответ на сообщение при этом ничего не ждёт: свои вложения и вложения
            // реплая уезжают в запрос к модели сами по себе.
            // Постановка без блокировки: переполнение не смертельно, недоделанное подберёт подчистка.
            var job = new MediaRecognitionJob(storedMessage);
            if (!_mediaRecognitionQueues.TryEnqueue(storedMessage.ChatId, job))
            {
